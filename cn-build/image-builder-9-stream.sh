#!/bin/bash
# CENTOS-9-STREAM BUILDER for CN
set -euo pipefail

source ${0%/*}/centos-stream-9-config.sh
...
if [[ ! -e ${NAME}-${MAJOR_RELEASE}-${DATE}.txt ]]
then
...
ImageId=$(aws ec2 register-image --region $REGION --architecture=$ARCHITECTURE \
	      --description="${NAME}-${MAJOR_RELEASE}.${MINOR_RELEASE} ($ARCH) for HVM Instances" \
...
echo "SNAPSHOT : ${snapshotId}, IMAGEID : ${ImageId}, NAME : ${IMAGE_NAME}" >> ${NAME}-${MAJOR_RELEASE}-${MINOR_RELEASE}.txt
